...
date: 2022-03-27
modified: 2026-08-11
excerpt: 推荐 Windows 系统下常用、好用的软件
category:
    - windows
tag:
    - 软件
...
在 [默认的下载页面](https://www.firefox.com.cn/download/) 下载的安装包默认是 32 位的，上述页面允许自行选择浏览器版本、运行平台、首选语言。
...
[PDF Arranger](https://github.com/pdfarranger/pdfarranger)![最新版本](https://img.shields.io/github/v/release/pdfarranger/pdfarranger)：用于合并或拆分 PDF 文档，并旋转、裁剪和重新排列页面。如果没有编辑文字的需求，相比于 Adobe Acrobat 更加轻巧（v1.13.0 约 58MB）。

> [PDF4QT](https://github.com/JakubMelka/PDF4QT) 项目虽说有编辑功能，但实际体验下来并不好用，可能是不支持中文文字。页面管理功能也不如 PDF Arranger 方便。

[稻壳阅读器 DocBox](https://www.52pojie.cn/thread-2063609-1-1.html)：仅 5.4MB，支持检索、阅读 PDF、XDF、XPS、EPUB、MOBI、AZW/AZW3、CAJ、OFD、DJVU、CBZ、CBR、CHM 等 30 多种格式的文档。但实测无法转换格式（如 ofd 转 pdf）

[ofd 文件阅读器](https://inv-veri.chinatax.gov.cn/xgxz.html)：增值税电子发票版式文件阅读器。40MB 左右~~，但该网站 SSL 证书有问题，显示不安全~~。
...
[Textify](https://ramensoftware.com/textify)![版本](https://img.shields.io/github/v/tag/m417z/Textify?label=Release&color=blue)：一款适用于 Windows 的工具，可以从原本不允许复制文本的对话框和控件中复制文本。（比如应用程序窗口标题、VB Script 的弹窗），采用鼠键结合进行复制。

> GitHub 地址：<https://github.com/m417z/Textify>
...
[魔曰（Abracadabra）](https://github.com/SheepChef/Abracadabra)：挺有意思的古文风文本加密工具，将数据加密为汉字构成的文言文，完全开源，易于部署，易于使用。在线体验：[https://abra.js.org/](https://abra.js.org/)
...
[完美解码](https://jm.wmzhe.com/)：一款能实现各种流行视频、HDTV 播放及编码的全能型影音解码包，自带 Media Player Classic、KMPlayer、PotPlayer 三款流行播放器，能播放众多种格式的影音文件。完全免费无广告！
...
[ShanaEncoder](https://github.com/1265578519/ShanaEncoder) ![最新版本](https://img.shields.io/github/v/release/1265578519/ShanaEncoder)：视频压制软件，支持 CPU 和 GPU 同时工作，速度快，清晰度高。不过上次更新是一年以前（应该是搬运的缘故）。
...
[Snipaste](https://zh.snipaste.com/)：~~开源、免费的~~截图工具，使用比较方便，支持贴图等功能。现在也出了专业版（￥99 大版本买断），Snipaste 2.x 对**个人用户**免费，但不会再增加新功能。

[ShareX](https://getsharex.com/)![最新版本](https://img.shields.io/github/v/release/ShareX/ShareX)：开源、免费的截图工具，支持录制 Gif 动图，但使用起来没有 Snipaste 方便。支持添加 [图片效果](https://getsharex.com/image-effects/)，个人使用后感觉添加水印比较难用。
...
<a id="Krokiet"></a>
[Czkawka / Krokiet](https://github.com/qarmin/czkawka/releases) ![最新版本](https://img.shields.io/github/v/release/qarmin/czkawka)：由 Rust 编写，能快速查找重复文件、相似图像/视频/音频。Czkawka 和 Krokiet 是两套不同 GUI。`windows_czkawka_gui_gtk_412.zip` 解压后使用。krokiet 选择 `_skia_opengl.exe` 版本（打开速度比其他略快），直接双击运行。
...
> 查找相似视频需要额外安装 FFmpeg。
> [releases 12.0.1](https://github.com/qarmin/czkawka/releases/tag/12.0.1) 已经宣布 12.0 版本是 Czkawka GTK 的最后一个发布版本，后续不会再发布新的二进制文件。

[QuickLook](https://quicklook.cc/)![最新版本](https://img.shields.io/github/v/release/QL-Win/QuickLook)：将 macOS 广受好评的 **Quick Look** 功能带到 Windows。无需打开任何软件，在文件资源管理器中按下空格键，即可闪电般预览文件内容。完全免费，开源透明，极致轻量。（插件 GitHub 列表：[可用插件](https://github.com/QL-Win/QuickLook/wiki/Available-Plugins)）
...
> 该软件是付费的，但拥有 30 天免费试用。Win+E 打开速度比 OneCommander 快。本体是挺轻量的（40 M+），但额外包含了 Everything、BandiZip、BandiView、NotePad++ 等软件（默认放在 `%AppData%\AllenExplorer`），所以安装包达到了 70M+。支持下载 QuickLook、AnyText Searcher 等强大软件，也可以手动选择路径。不过压缩软件绑死 BandiZip，且不支持手动选择、配置其他压缩软件。

[OneCommander](https://one-commander.com/)：高性能的 Windows 资源管理器替代品，采用现代化的双窗格文件管理器架构，配合自适应的 Miller 列视图、内置多标签页与 DirectX 加速渲染界面，让文件导航焕然一新。**个人使用免费**，商用需购买 Pro 版本。

> 不过启动依旧不是秒开，能计算文件夹大小挺方便，支持 QuickLook。
...
[Obsidian](https://obsidian.md/zh/) ![最新版本](https://img.shields.io/github/v/release/obsidianmd/obsidian-releases)：能当做 markdown 编辑器，还可以做知识管理。

[微信 markdown 编辑器](https://doocs.gitee.io/md/) 在线工具，可将 markdown 内容进行排版、美化，然后复制并发布到公众号文章。

[猎豹护眼大师](http://www.downxia.com/downinfo/175516.html) ：此处是绿色精简免安装版（当下软件园）。

[景好鼠标录制器](https://zxapi.com/shubiao/)：可以录制鼠标动作并重新播放的免费绿色软件。

[深蓝词库转换](https://github.com/studyzy/imewlconverter) ![最新版本](https://img.shields.io/github/v/release/studyzy/imewlconverter)：一款输入法词库转换软件，支持以下超过 20 种的输入法工具和词库。

[手心输入法](https://www.xinshuru.com/)：无广告、不捆绑第三方的输入法，也是 360 出品，电脑端相对好用，可以代替某狗。

[eza](https://github.com/eza-community/eza/releases)![最新版本](https://img.shields.io/github/v/release/eza-community/eza)：一个用 Rust 编写的、现代化的、维护良好的 `ls` 替代工具，并提供了更多功能和更优的默认设置。它使用颜色来区分文件类型和元数据。它支持符号链接、扩展属性和 Git。而且它**体积小**、**速度快**，并且只有**一个二进制文件**。
...
[Geek Uninstaller](https://geekuninstaller.com/download) ：卸载软件很方便，Pro 版本可以批量卸载软件，但免费版足矣。

[HiBit Uninstaller](https://www.hibitsoft.ir/Uninstaller.html)：彻底卸载顽固软件、Windows 应用和浏览器扩展程序。

[驱动总裁](https://www.sysceo.com/dc)：适用于重装电脑后更新驱动，下载速度快，驱动稳定。

[Windows11 轻松设置](https://www.bilibili.com/opus/904672369138729017)：支持移除系统自带软件（尤其是 Edge 浏览器），其他各种小功能。首发在远景论坛，但有注册门槛。

[CrystalDiskInfo](https://www.crystaldiskinfo.cn/)：免费开源的硬盘健康监测工具，支持 HDD/SSD 硬盘 SMART 信息查看、温度监控和健康状态预警。提供详细硬盘信息，预防数据丢失。

> GitHub 地址：<https://github.com/hiyohiyo/CrystalDiskInfo>

[360 加速球绿色独立版](https://360-tray.github.io/)：保留加速球功能

[HEVC 视频扩展](https://apps.microsoft.com/detail/9n4wgh0z6vhq?hl=zh-CN&gl=CN)：使 Windows 系统支持显示 HEVC 编码视频的缩略图。
