...
下面可以通过实现一个简单的 HTTP 服务器了解 Go 语言到底有多么简单：
...
```go
fmt.Println(s[2:5]) // [c d e]，第2个到第5个位置的元素，不包括第5个元素
fmt.Println(s[:5])  // [a b c d e]，从头到第5个位置的元素，不包括第5个元素
fmt.Println(s[2:])  // [c d e f]，从第2个到最后一个元素，不包括第2个元素
```
