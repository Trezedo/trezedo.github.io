...
当插入第 $i(i>=1)$ 个元素时，前面的 $arr[0], arr[1] ,\cdots, arr[i-1]$ 已经排好序（相对顺序），此时用 arr[i] 与 $arr[i-1], arr[i-2],\cdots$ 依次分别比较，找到插入位置就将 $arr[i]$ 插入，原来位置上的元素顺序后移。

![插入排序思想示意图 |360](https://zedo-img.netlify.app/img/dsa/sorts/1.1-插入排序.png)

### 代码实现

假设 $\rm [0, end]$ 有序，将把 $\rm end+1$ 的值插入，让 $\rm [0, end+1]$ 有序，我们要做的就是让 $\rm end$ 不断变大，然后重复以上过程。

注意 $\rm end+1$ 不能越界，范围是 $[1, n)$，于是 $\rm end$ 范围是 $[0, n-1)$。
...
先选定一个整数 $\text{gap}$，把待排序文件中所有记录分成 $\text{gap}$ 个组，所有距离为 $\text{gap}$ 的记录分在同一组内，并对每一组内的记录进行排序。然后取 $\text{gap}=gap/2$，重复上述分组和排序的工作。当 $\text{gap}=1$ 时，所有记录在同一组内排好序。
...
- $\text{gap}$ 的选取没有“官方”规定，即希尔排序算法的增量序列取法不唯一，且其效率与增量序列的取法有关。
- 当 $\text{gap}>1$ 时都是预排序，目的是让数组更接近于有序。当 $\text{gap}==1$ 时，实际上就是直接插入排序。这样整体而言，可以达到优化的效果。
...
- 预排序：分组排序，间隔为 $\text{gap}$ 为一组，分别进行直接插入排序，$\text{gap}$ 由大变小。
- 目的：$\text{gap}$ 越大，大数就能更快地排在后面，小数更快地排在前面。但 $\text{gap}$ 越大，预排完越不接近有序，$\text{gap}$ 越小，越接近有序。
- 当 $\text{gap}=1$ 时就是直接插入排序。

每 $\text{gap}$ 为一组，增量为 $d = \text{gap}$，变成逻辑上的分组：$[i, i+d, i+2d, ..., i+kd]$

与直接插入排序的实现类似，现在还需要确定 $\rm end$ 变量的范围。

因为每 $\text{gap}$ 个一组，最后 $\text{gap}$ 个元素必定位于不同组，且倒数第 $\text{gap}$ 个元素与 $arr[0]$ 同组，即从 $arr[n-\text{gap}]$ 开始，每个元素都是它们组最后一个元素，从而可确定 $\rm end$ 范围是 $[0, n - \text{gap})$。
...
希尔排序的时间复杂度不好计算。当 $\text{gap}$ 很大时，复杂度约为 $O(n)$，当 $\text{gap}$ 很小时，数组已经接近有序，也差不多是 $O(n)$。平均时间复杂度 $O(n^{1.25})\sim O(1.6n^{1.25})$，一般取为经验值 $O(n^{1.3})$。
...
- $\{1,5,19,41,109,\cdots\} =\{k\mid (9(4^k-2^k)+1) \lor (4^k-3\cdot2^k+1)\}$
...
我们在实现直接插入排序时，是这样做的：假设 $[0, end]$ 有序，将把 $end+1$ 的值插入，让 $[0, end+1]$ 有序。

但这样每次取 `tmp` 时都是从 `end+1` 开始，其实也可以把 `end` “平移”一下，即：
