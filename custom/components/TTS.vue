<template>
    <p v-show="disabled" style="color: red;font-weight: bold"> 抱歉，当前浏览器不支持语音合成API。</p>
    <div>
        <label>语言：</label>
        <!--suppress HtmlUnknownAttribute -->
        <select :disabled="disabled" v-model="currentLang">
            <option selected value="all">全部</option>
            <option v-for="lang in getSupportedLang()" :value="lang">{{ lang }}</option>
        </select>
    </div>
    <div>
        <label>音色：</label>
        ...
        <select :disabled="disabled" v-model="speakerIdx">
            <option v-for="(voice, index) in filterVoicesByLang(currentLang)" :value="index">
                {{ voice.name }}
            </option>
